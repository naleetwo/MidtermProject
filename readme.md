...
- 환경적 요인에 대한 분석 결과, 편의점, 카페, 학원 수가 가장 큰 영향을 미치나. 아파트 매매가와 전월세를 기준으로 한 설명력이 다소 부족합니다.(매매가 기준=R^2=34.6%, 전월세 R^2=41.3%). 따라서 환경적 요인 분석 결과를 해석할 때에는 다른 변수들과 함께 종합적으로 고려해야 합니다.

#### 7. 자체 평가 의견
- 경제적 요인은 시간에 따라 어떤 경향성을 띄는지 확인하고 아픙로의 아파트 가격을 예측하기 위해 시계열분석을 사용하고 싶었지만 분석모델에 대한 학습이 부족하여, 다중회귀분석만을 사용하여 분석하여 분석모델의 한계가 있었습니다.
- 시계열 분석 모델을 머신러닝으로 구축하고 분석하는 방ㅂ버을 사용하면 아파트 가격을 예측하는 모델을 찾을 수 있을 것 같습니다.
